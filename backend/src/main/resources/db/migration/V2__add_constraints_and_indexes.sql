...
--
-- NOTE:
-- This migration is intentionally a no-op now.
-- The original index statements caused Flyway startup failures on Railway MySQL after a partial failed V2 run.
-- Indexes are performance-only here, so leaving this migration successful is safer than blocking application startup.
